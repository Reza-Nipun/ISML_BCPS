...
<div class="row">
 ...
 <span style="color:#0F9; font-size:14px">The Auto Cut Id is : </span><u><span style="color:#0FF; font-size:18px">&nbsp;<?php echo $get_cid; ?> </span></u>
							...
							<?php if($get_ttl != '') { ?>
 &nbsp;<span style="color:#0F9; font-size:14px"> And Total Qty is : </span><u><span style="color:#0FF; font-size:18px">&nbsp;<?php echo $get_ttl; ?> </span></u>
							<?php } ?>
    ...
    <summary><span style="font-size:14px; font-weight:bold; color:#0190b8">Search Auto Cut ID &#10144; </span></summary>-->
         ...
         <form name="Form1" action="cutting_input_print.php" method="post" class="form-inline" role="form">
     ...
     <?php } else { ?> 
		...
		/*$part_array = array();
		$SQL_part = "SELECT PartName FROM tb_vsfs_bundle_info WHERE CutID = '$get_cid' GROUP BY PartName";
		$obj->sql($SQL_part);
		while($row = mysql_fetch_array($obj->result))
		{ 
		$part_array[] = $row['PartName'];
		  ...
		  $SQL2 = "SELECT T0.PartName, T0.IsPrint FROM `tb_vsfs_part_info` T0 WHERE T0.`StyleCode` = '$StyleCode' ORDER BY PartInfoID ASC";

			*/
	
	 $SQL2 = "SELECT PartName, print_flag FROM tb_vsfs_bundle_info WHERE CutID = '$get_cid' GROUP BY PartName ORDER BY PartName ASC";
			  ...
			  <tr>
				  ...
				  <td><?php if($row2['print_flag'] == 1) echo '<span style="color:#0F9; font-weight:bold; font-size:15px">Yes</span>'; else echo 'No'; ?></td>
			  </tr>
	<?php } ?>               
		  </table> 
	   </div>
            	...
            	<td valign="top">
                	...
                	<table class="bottomBorder" border="1" style="margin-left:20px">
						...
						<?php 
                          ...
                          // $SQL2 = "SELECT Suff, LotNo FROM tb_vsfs_bundle_info WHERE CutID = '$get_cid' GROUP BY Suff ORDER BY AutoBundleID ASC";
                          $SQL2 = "SELECT Suff, LotNo FROM tb_vsfs_bundle_info WHERE CutID = '$get_cid' GROUP BY Suff ORDER BY LENGTH(Suff), Suff ASC";
						  $result2 = $obj->sql($SQL2);
                          while($row2 = mysql_fetch_array($result2))
                          {
                        ?>
