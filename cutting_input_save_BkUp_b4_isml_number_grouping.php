	  ...
	  while($mrkr <= $marker)
	  {
		...
		$temp_bndl=explode("+",$bundle_ratio);	// the Bundle Ration is divided here by + Sign.
			...
			foreach($temp_bndl as $value=>$bundle_1) // Here $bundle_1 is a group of bundle located in $bundle_ratio.
			{
				  ...
				  $SQL = "INSERT INTO `tb_vsfs_bundle_info` (`AutoBundleID`, `CutID`, `BundleNo`, `RollNo`, `PartName`, `Size`, `Suff`, `LotNo`, `BundleRange`, `Qty`, `print_flag`) VALUES ('', '$auto_CutID', '$bundle_no', '$roll_no', '$PartName', '$cut_size', '$size_main', '$lot_no', '$bundle_range', '$b_qty', '$IsPrint')";
				 	//die($SQL);
				  $obj->sql($SQL);
