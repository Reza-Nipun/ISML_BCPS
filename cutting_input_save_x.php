	 ...
	 /*foreach($_POST['part_info'] as $part_row=>$part_info)
	  ...
	  else
	  {
		  ...
		  { $auto_CutID = $row_CutID['auto_CutID']; } // May be it also can do with $SQL ;
			...
			$SQL = "INSERT INTO tb_vsfs_cut_info (`sl`, `AutoCutID`, `buyer`, `StyleCode`, `Color`, `CutNo`, `OrderNo`, `Lay`, `UnitName`, `u_id`, `entry_date`, `entry_time`) VALUES ('', '$auto_CutID', '$buyer', '$style', '$color', '$cut_id', '$order_no', '$lay', '$unit', '$vsfs_uid', '$date', '$datex')"; //here the special field is missing as it will autometically assigned as 0 bcz it is not for Special Cut Save.
			// die($SQL);
			$obj->sql($SQL);
	  }
	 ...
	 $bundle_no = 1 ;
	  ...
	  while($mrkr <= $marker)
	  {
		...
		$temp_bndl=explode("+",$bundle_ratio);	// the Bundle Ration is divided here by + Sign.
			...
			$SQL_Sticker = "INSERT INTO `tb_vsfs_sticker_info` (`sl`, `CutID`, `Size`, `Start`, `End`, `Qty`) VALUES ('', '$auto_CutID', '$size_main', '$sticker_st', '$bndl_end', ($bndl_end-$sticker_st+1))"; 
			$obj->sql($SQL_Sticker);
