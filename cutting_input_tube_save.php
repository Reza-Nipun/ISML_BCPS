	 ...
	 /*foreach($_POST['part_info'] as $part_row=>$part_info)
	  ...
	  else
	  {
		  ...
		  { $auto_CutID = $row_CutID['auto_CutID']; } // May be it also can do with $SQL ;
			...
			$SQL = "INSERT INTO tb_vsfs_cut_info (`sl`, `AutoCutID`, `buyer`, `StyleCode`, `Color`, `Clr_OrderQty`, `CutNo`, `OrderNo`, `Lay`, `UnitName`, `IsTube`, `u_id`, `entry_date`, `entry_time`) VALUES ('', '$auto_CutID', '$buyer', '$style', '$color', '$Clr_OrderQty', '$cut_id', '$order_no', '$lay', '$unit', '1', '$vsfs_uid', '$entry_date', '$datex')"; //here the special field is missing as it will autometically assigned as 0 bcz it is not for Special Cut Save.
			// die($SQL);
			$obj->sql($SQL);
	  }
		...
		$temp_bndl=explode("+",$bundle_ratio);	// the Bundle Ratio is divided here by + Sign.
			...
			foreach($temp_bndl as $value=>$bundle_1) // Here $bundle_1 is a group of bundle located in $bundle_ratio.
			{
				  ...
				  //$SQL = "INSERT INTO `tb_vsfs_bundle_info` (`AutoBundleID`, `CutID`, `BundleNo`, `PartName`, `Size`, `Suff`, `LotNo`, `BundleRange`, `Qty`, `print_flag`) VALUES ('', '$auto_CutID', '$bundle_no', '$PartName', '$cut_size', '$size_main', '$lot_no', '$bundle_range', '$b_qty', '$IsPrint')";
				 	//die($SQL);
				  $SQL = "INSERT INTO `tb_vsfs_bundle_info` (`AutoBundleID`, `CutID`, `BundleNo`, `RollNo`, `PartName`, `Size`, `Suff`, `LotNo`, `BundleRange`, `Qty`, `print_flag`) VALUES ('', '$auto_CutID', '$bundle_no', '$roll_no', '$PartName', '$cut_size', '$size_main', '$lot_no', '$bundle_range', '$bundle_qty_m', '$IsPrint')";
				  $obj->sql($SQL);
